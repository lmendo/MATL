...
A ``do twice'' loop is begun by \matl{,} and ended by \matl{]}. The statement \matl{,} is equivalent to \matl{FT"}; that is, it produces a ``for'' loop with two iterations. The statements \matl{.}, \matl{X.} and \matl{@} can be used as in a ``for'' loop. In particular, \matl{@} will push \matl{F} in the first iteration and \matl{T} in the second.
