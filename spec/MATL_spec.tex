...
\item
When using Octave's \matlab+vpa+, conversion from symbolic to char looses precision. This is solved by converting the output of Octave's \matlab+vpa+ to string using \matlab+pretty+ (instead of \matlab+char+). This introduces unwanted whitespace in the beginning, middle and end of the string. There may also be wanted whitespace separating elements of an array. But it is difficult to distinguish that wanted space from unwanted one. So all whitespace is removed, which only works for scalar data.
...
Consequently, \matlab+vpa(str2sym(...), ...)+ is needed for char input. On the other hand, Octave does not have \matlab+str2sym+, and allows char input to \matlab+vpa+. This is handled by the compiler by using \matlab+vpa(str2sym(...), ...)+ for char input in the compiled code but defining \matlab+str2sym+ as the identity function in Octave.
\item
...
% Cuando Octave tenga str2sym se podrá hacer esa conversión. Y entonces habrá que quitar la definición actual de str2sym como identity function, que ya no sería necesaria para vpa (e impediría hacer conversiones de string a symbolic en general)
\item
