...
\begin{pending}
The symbolic data type was not originally in the language; it was a later addition, suggested by \user{@Sanchises}.
...
\parte{Separators} are sometimes needed to separate literals of the same type. For example, \matl{12} means the number $12$, whereas \matl{1 2} means number $1$ followed by number $2$. Similarly, \matl{'Padm{\'e}' 'Anakin'} means string \matl{'Padm{\'e}'} followed by string \matl{'Anakin'}, whereas \matl{'Padm{\'e}''Anakin'} would be interpreted as a single string containing an apostrophe between the two names (quotation marks within strings are duplicated, as in MATLAB).
...
\begin{pending}
Using for example \matl{H} instead of \matl{2} may be useful to avoid a separator (space or newline) if that $2$ is surrounded by other numbers. Values $0$ and $1$ can be obtained with functions \matl{O} and \matl{l} (which correspond to \matlab+zeros+ and \matlab+ones+ respectively; by default they take no inputs and produce a single number as output).
\end{pending}
...
\item
When using Octave's \matlab+vpa+, conversion from symbolic to char looses precision. This is solved by converting the output of Octave's \matlab+vpa+ to string using \matlab+pretty+
% which actually calls `disp`
(instead of \matlab+char+). This introduces unwanted whitespace in the beginning, middle and end of the string (example: \matlab+pretty(vpa(1, 100))+). There may also be wanted whitespace separating elements of an array (example: \matlab+pretty(vpa(pi:5, 100))+) or with complex values. It is difficult to distinguish wanted space from unwanted one. So, for the scalar, real-valued case all whitespace is removed (because all is unwanted). In any other case the whitespace is not removed (because some is wanted).
...
% Cuando Octave tenga str2sym quizá se podrá hacer esa conversión.
\item
...
\item
\matlab+hypergeom+ doesn't exist in Octave. This is partly addressed by the compiler. The function is computed either by direct sum (when some of the numerator coefficients is a non-positive integer, in which case the sum is finite) or using a third-party function\footnote{\hspace{2mm}\url{http://www.mathworks.com/matlabcentral/fileexchange/5616-generalized-hypergeometric-function}}, which only works for certain ranges of input values.
\item
Octave's \matlab+disp+, unlike MATLAB's, produces some output for empty input. This has been fixed. Correcting this requires intercepting \matlab+disp+ in Octave. This prevents the \matlab+disp+ function for \matlab+sym+ values from being called\footnote{ At least I don't know how to do it. See  \url{https://stackoverflow.com/questions/58727452/how-to-call-a-shadowed-function-in-octave}
}. To solve this, the code of \matlab+disp+ for \matlab+sym+ values is manually included. This is not an elegant solution, but as a side effect it allows some tweaking, namely removing left indentation which is introduced by Octave and not by MATLAB. 
\item
...
\item
Octave's \matlab+ismember+, \matlab+mean+, \matlab+svd+, \matlab+gcd+, \matlab+lcm+ don't work with symbolic data. This hasn't been fixed.
\item
