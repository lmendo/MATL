...
\matl{Y"} & 2-- (2 / 3) & 1 & \matlab+repelem+ (run-length decoding) \\
...
\matl{Z\$} & 0--1 (0 / 1) & 1 & Reads bytes from specifed file. The output is a row vector of char. If $0$ inputs or empty input: file name is \comp{inout}. \\
...
\matl{Z+} & 2 & 1 & \matlab+conv2(..., 'same')+. Converts inputs to \matlab+double+. \sa \matl{Y+} \\
...
\matl{X-} & 2--4 (2 / 3) & 1--2 (1) & \matlab+setdiff+. Uses the \matlab+'stable'+ flag by default. If one input is char and the other is numeric, the latter is converted to char. This function allows flag strings in third and subsequent inputs to be replaced by numbers, as follows: 1: \matlab+'rows'+, 2: \matlab+'stable'+, 3: \matlab+'sorted'+ \\
\matl{Y-} & 2 & 1--2 (2 / 1) & \matlab+deconv+ \\
...
\matl{YD} & 1-- (2) & 0--2 (1) & \matlab+sprintf+. If $0$ outputs: prints to screen using \matlab+fprintf(...)+. \sa \matl{D}, \matl{XD}, \matl{ZD} \\
...
\matl{YF} & 1 & 1--2 (1 / 2) & (i) With $1$ output: exponents of prime factor decomposition, without skipping primes. If the input is a numeric array it is linearized, and each result is in an output row. (ii) With $2$ outputs: first output gives the prime factors, second is as before. \sa \matl{Yf} \\
...
\matl{XG} & 1-- (1 / 2) & 0 & \matlab+plot+. Calls \matlab+drawnow+ to update figure immediately. With one input, if the input is complex (even with zero imaginary part), \matlab+axis equal+ is also called. \\
...
\matl{XL} & 1-- (2 / 3) & 0 & copy to multi-level clipboard L. Topmost input specifies level \\
...
\matl{Xd} & 1--4 (1 / 2) & 1--2 (1) & If $1$ input and $1$ output: \matlab+diag+. Otherwise: \matlab+spdiags+, with char inputs automatically converted to \matlab+double+ \\
...
\matl{Xh} & 0-- ($^\ddagger$) & 1 & concatenate into cell array (\matlab+{..., ...}+) \\
...
\matl{Xi} & 1--5 (1) & 1--2 (1) & \matlab+urlread+ \\
...
\matl{r} & 0-- (0) & 1 & \matlab+rand+ \\
\matl{Xr} & 0-- (0) & 1 & \matlab+randn+ \\
\matl{Yr} & 1-- (1 / 2) & 1 & \matlab+randi+. With $2$ inputs, if the second input is a scalar \matlab+n+ it is transformed into \matlab+[1 n]+ \\
...
\matl{Zs} & 1--4 (1 / 2) & 1 & \matlab+std+. With $2$ inputs \matlab+x+, \matlab+f+ and \matlab+f+ equal to $2$ or $3$, gives \matlab+cov(x, f-2)+ \\
...
\matl{Z|} & 2--3 (2) & 1 & \matlab+bitor+, element-wise with singleton expansion. If first or second inputs are \matlab+char+ they are converted to \matlab+double+. \matlab+double+ inputs are rounded. If the inputs are \matlab+double+ they can have negative values, and in that case they must be in the range from \matlab+-2^52+ up to \matlab+2^52-1+. \sa \matl{Z\&}, \matl{Z\textasciitilde{}} \\
...
\matl{X\textasciitilde{}} & 2--4 (2 / 3) & 1--3 (1) & \matlab+setxor+. Uses the \matlab+'stable'+ flag by default. If one input is char and the other is numeric, the latter is converted to char. This function allows flag strings in third and subsequent inputs to be replaced by numbers, as follows: 1: \matlab+'rows'+, 2: \matlab+'stable'+, 3: \matlab+'sorted'+ \\
\matl{Y\textasciitilde{}} & 2 & 1 & \matlab+xor+, element-wise with singleton expansion \\
\matl{Z\textasciitilde{}} & 2--3 (2) & 1 & \matlab+bitxor+, element-wise with singleton expansion. If first or second inputs are \matlab+char+ they are converted to \matlab+double+. \matlab+double+ inputs are rounded. If the inputs are \matlab+double+ they can have negative values, and in that case they must be in the range from \matlab+-2^52+ up to \matlab+2^52-1+. \sa \matl{Z\&}, \matl{Z|} \\
