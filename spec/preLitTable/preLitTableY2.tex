\begin{longtable}{|p{.03\textwidth}|p{.29\textwidth}||p{.03\textwidth}|p{.29\textwidth}||p{.03\textwidth}|p{.29\textwidth}|}
\hline
\multicolumn{6}{|c|}{Function \matl{Y2}} \\ \hline\hline
\matl{1} & \matl{'A':'Z'} &
\matl{2} & \matl{'a':'z'} &
\matl{3} & \matl{['A':'Z' 'a':'z']} \\ \hline
\matl{4} & \matl{'0':'9'} &
\matl{5} & \matl{['0':'9' 'A':'F']} &
\matl{6} & \matl{' ':'\textasciitilde{}'} \\ \hline
\matl{7} & \matl{[' ':'\&' '(':'\textasciitilde{}']} &
\matl{8} & \matl{['0':'9' 'A':'Z' 'a':'z']} &
\matl{9} & \matl{['0':'9' 'A':'Z' '\_' 'a':'z']} \\ \hline
\matl{10} & \matl{['0':'9' 'A':'Z']} &
\matl{11} & \matl{'aeiou'} &
\matl{12} & \matl{'AEIOU'} \\ \hline
\matl{13} & \matl{'aeiouAEIOU'} &
\matl{14} & \matl{'bcdfghjklmnpqrstvwxyz'} &
\matl{15} & \matl{'BCDFGHJKLMNPQRSTVWXYZ'} \\ \hline
\matl{16} & \matl{'bcdfghjklmnpqrstvwxyzBCDFGHJKLMNPQRSTVWXYZ'} &
\matl{17} & \matl{'aeiouy'} &
\matl{18} & \matl{'AEIOUY'} \\ \hline
\matl{19} & \matl{'aeiouyAEIOUY'} &
\matl{20} & \matl{['Mon'; 'Tue'; 'Wed'; 'Thu'; 'Fri'; 'Sat'; 'Sun']} &
\matl{21} & \matl{[298 302 288 305 289 296 310]} \\ \hline
\matl{22} & \matl{\{'January' 'February' 'March' 'April' 'May' 'June' 'July' 'August' 'September' 'October' 'November' 'December'\}} &
\matl{23} & \matl{\{'Monday' 'Tuesday' 'Wednesday' 'Thursday' 'Friday' 'Saturday' 'Sunday'\}} &
& \\ \hline
\end{longtable}
